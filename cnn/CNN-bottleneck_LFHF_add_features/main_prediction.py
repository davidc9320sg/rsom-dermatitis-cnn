...
class PrepareData:
    ...
    def assign_label(self, label, experiment_type):
        if experiment_type == 'healthy_vs_disease':
           y = 0 if (label == 'mild') or (label == 'moderate') or (label == 'severe') else 1
        if experiment_type == 'mild_vs_modsev':
           y = 0 if (label == 'mild') else 1
    ...
    def _load_additional_npy(self, experiment_type):
        ...
        for LFHF_file_tmp in study_file_list:
            ...
            #fea = fea_df[fea_df['case'] == casename].iloc[:, -4:].values.squeeze()
            fea = fea_df[fea_df['case'] == casename][['TEWL_norm', 'ET_norm', 'TBV_norm', 'LHFR_norm']].values.squeeze()
            img_list.append(img)
    ...
    def _load_feature_data(self, no_sample, severity_type):
        ...
        #feature_df = df_specific_severity.iloc[:, -4:]
        feature_df = df_specific_severity[['TEWL_norm', 'ET_norm', 'TBV_norm', 'LHFR_norm']]
...
if __name__ == '__main__':
    ...
    valid_folder_to_read = '../../data/healthy_v_eczema/CV{}/{}/'.format(cv, experiment_type)
    loss_cv_folder = 'results/healthy_v_eczema/CV{}/{}/'.format(cv, experiment_type) 
    checkpoint_dir = 'results/healthy_v_eczema/CV{}/{}/model/my_model.ckpt-{}'.format(cv, experiment_type, epoch_to_load)
    ...
    CNN_name = "rsom"
    my_cnn = CNN(CNN_name)
    model_output = my_cnn._build_model(X, features)
    ...
    with tf.Session() as sess:
        ...
        for i in range(len(X_valid) // VALID_BATCH_SIZE):
            ...
            val_prediction_one_hot = sess.run(model_output, feed_dict={X:X_valid_tmp, features:fea_valid_tmp})
            ...
            val_acc = sess.run([accuracy], feed_dict={X:X_valid_tmp, y:y_valid_tmp, features: fea_valid_tmp})
        ...
        error = abs(epoch_val_acc - loaded_epoch_val_acc)/loaded_epoch_val_acc * 100
        if error > 20:  # if error > 20%
           raise ValueError('val_acc: {} computed using loaded model does not match with val_acc: {}  read from csv. Model may not loaded correctly'.format(epoch_val_acc, loaded_epoch_val_acc))
        ...
        for n in range(len(X_test) // TEST_BATCH_SIZE):
            ...
            test_prediction_one_hot = sess.run(model_output, feed_dict={X:X_test_tmp, features:fea_test_tmp})
    ...
    test_df = pd.DataFrame(columns=['test_case', 'test_softmax_0', 'test_softmax_1', 'test_prediction', 'test_ground_truth'])
    ...
    test_df['test_case'] = test_casename
    test_df[['test_softmax_0', 'test_softmax_1']] = test_prediction_one_hot_arr
    test_df['test_prediction'] = test_prediction_int_arr
    test_df['test_ground_truth'] = test_y_ground_truth
